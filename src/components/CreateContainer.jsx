...
const CreateContainer = () => {
  ...
  const uploadImage = (e)  => {
    ...
    uploadTask.on('state_changed', (snapshot) => {
      ...
      console.log(error);
      setFields(true);
      setMsg('Error al subir imagen : intenta de nuevo 🙇‍♀️!');
  ...
  const saveDetails = () => {
    ...
    try{
      if((!title || !calories || !imageAsset || !price || !categories)){
        setFields(true);
        setMsg('¡Se requieren todos los datos solicitados 🙇‍♀️!');
        setAlertStatus('danger')
        setTimeout(() => {
        setFields(false)
        setIsLoading(false)
        ...
        saveItem(data) // esto es una promesa del firebase
        setIsLoading(false)
        setFields(true);
        setMsg('Item añadido con éxito 😊!');
        clearData();
        setAlertStatus('succes');
        setTimeout(() => {
          setFields(false);
      ...
      console.log(error);
      setFields(true);
      setMsg('Error al subir cambios: intenta de nuevo 🙇‍♀️!');
      setAlertStatus('danger')
      setTimeout(() => {
        setFields(false)
        setIsLoading(false)
  ...
  const clearData = () => {
    ...
    setCalories('');
    setPrice('');
    setCalories('Seleccionar Categoría');
  }
      ...
      <div className='w-[90%] md:w-[75%] border border-gray-200 rounded-lg p-4 flex flex-col items-center justify-center gap-4'>
        {
          ...
          <div className='group flex justify-center items-center flex-col border-2 border-dotted border-gray-300 w-full h-255 
          md:h-420 cursor-pointer rounded-lg'>
            {
                ...
                <label className='w-full h-full flex flex-col items-center justify-center cursor-pointer'>
                  <div className='w-full h-full flex flex-col items-center justify-center gap-2'>
                    ...
                    <p className='text-gray-500 hover:text-gray-700'>Subir imagen</p>
                  </div>
              ...
              <><div className='relative h-full'>
                <img src={imageAsset} alt="subir imagen" 
